...
layout (rgba16f) uniform image2D waveSim2;
    ...
    layout (r32ui) uniform readonly uimage2D entityStampImg;
#endif
...
void main() {
    ...
    #ifdef ENTITY_WATER_RIPPLES
        bool entityRippleWake = entityRippleWakeSSBO;
    #else
        const bool entityRippleWake = false;
    #endif

    if (abs(frameTimeCounter - lastFrameTimeCount) > WATER_SIM_FRAMETIME && (!noSimOngoing || onWaterSurface || entityRippleWake)) {
        ...
        #ifdef ENTITY_WATER_RIPPLES
            // the sim was already scrolled into the new alignment by prepare1, but the
            // entity stamps are still in last frame's alignment. compensate manually.
            ivec2 entityStampCoord = clamp(imgCoord - water_move_compensationSSBO, ivec2(1), resolution - ivec2(1));
            float entityStamp = float(imageLoad(entityStampImg, entityStampCoord >> 1).r) * (1.0/255.0);
            pressure += entityStamp * ENTITY_RIPPLE_STRENGTH;
        #endif
