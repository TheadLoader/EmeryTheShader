...
void main() {
    ...
    if (abs(frameTimeCounter - lastFrameTimeCount) > WATER_SIM_FRAMETIME && (!noSimOngoing || onWaterSurface || entityRippleWake)) {
        ...
        #ifdef ENTITY_WATER_RIPPLES
            // entity/item stamps were written last frame in the sim's old alignment,
            // so read them at sampledCoord. the stamp image is half the sim resolution.
            float entityStamp = float(imageLoad(entityStampImg, sampledCoord >> 1).r) * (1.0/255.0);
